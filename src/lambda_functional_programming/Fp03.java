...
public class Fp03 {
    ...
    public static void uzunlugaGoreYazdir(List<String> list) {
        ...
        list.stream().// sorted() sadece bunu dersek alfabetik siralama yapar
                sorted(Comparator.comparing(String::length)).//Comparator.comparing(): Sıralama koşulunu belirtmek için kullanılır
    ...
    list.stream().
            sorted(Comparator.comparing(String::length).reversed()).//reversed() tersden uzunlugu siralar
            forEach(Utils::ayniSatirdaBosluklaYazdir);
    ...
    public static void uzunlukVeIlkHarfeGoreSiralaYazdir(List<String> list){
        ...
        list.stream().                                                   // (t->t.charAt(0)
                sorted(Comparator.comparing(String::length).thenComparing(Utils::ilkKarekteriAl)).//thenComparing() :==>siralama icin bir kosul daha belirtir
                                                                                                    //daha sonra sirala da diyebiliriz
                forEach(Utils::ayniSatirdaBosluklaYazdir);
...
//
   //     list.removeIf(t->t.length()>5);             ==> list, mutable olduğu için elemanları işlem sonrası kalıcı değiştiğinden
    //                                                        comment-out yapıyorum.( yoruma aliyorum )
//
   //     System.out.println(list);
//
//
//
 ...
 //       // removeIf() eger kosul dogru ise sil
 ...
 // }
    ...
    public static boolean uzunlugu12denAzMi(List<String> list){
        ...
        return list.stream().allMatch(t->t.length()<12);// allMatch hepsinin kosulu saglayip saglamadigi
                                                         //  anyMatch her hangibiri kosulu saglayip saglamadigi
                                                         // noneMatch hicbiri  kosulu saglayip saglamadigi
    }
    ...
    public static boolean xIleBaslamiyorMu(List<String> list){
       ...
       // return list.stream().noneMatch(t->t.charAt(0)=='x'|| t.charAt(0)=='X');
